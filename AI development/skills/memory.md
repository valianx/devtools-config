...
1. Use Memory MCP `search_nodes` with the query
...
1. Use Memory MCP `read_graph` to get all entities
...
1. Use Memory MCP `open_nodes` with the entity name
...
1. Use Memory MCP `read_graph` to get everything
2. Display:
   ```
...
5. If user confirms → use Memory MCP `delete_entities` for each confirmed entity

### `consolidate` — Merge similar entities

1. Use Memory MCP `read_graph`
...
- If Memory MCP is not available → "Memory MCP server is not running. Check your Claude Code MCP configuration."
...
- Uses Memory MCP tools directly: `search_nodes`, `read_graph`, `open_nodes`, `create_entities`, `add_observations`, `delete_entities`, `delete_observations`, `create_relations`, `delete_relations`
