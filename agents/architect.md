...
Used when the team needs acceptance criteria derived from a PR and its linked issue, for external PRs that don't have session-docs.

- **Trigger:** orchestrator invokes with "pr-analysis mode"
- **Input:** PR title, body, linked issue body, diff summary
- **Output:** AC list in Given/When/Then format (returned to orchestrator, not written to session-docs)
- **Flow:** Read PR context → generate AC → return to orchestrator

This is a lightweight mode. You do NOT produce an architecture proposal. You analyze the intent of the PR (from issue description and diff) and produce 5-15 testable acceptance criteria.

**This mode skips Session Context Protocol and Execution Log Protocol** — there are no session-docs involved. You receive context directly from the orchestrator and return results directly.
...
#### Return Protocol (PR Analysis)

```
agent: architect
status: success | failed
output: pr-analysis (inline)
summary: generated {N} acceptance criteria from PR #{number}
ac_list:
1. Given {context}, When {action}, Then {expected result}
2. ...
