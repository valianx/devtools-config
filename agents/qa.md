...
Used inside the review flow to validate acceptance criteria against a PR's diff, without session-docs.

- **Trigger:** orchestrator invokes with "validate-pr mode"
- **Input:** AC list (generated by architect from PR/issue context) + PR base/head branches
- **Flow:** Phase 0 (context) → read AC from input → read PR diff via `git diff origin/{base}...origin/{head}` + read changed files → validate each AC → return results
- **Output:** AC validation results returned to orchestrator (not written to session-docs)

**This mode skips Session Context Protocol and Execution Log Protocol** — there are no session-docs involved. You receive AC and branch info directly from the orchestrator and return results directly.

#### Validation Process (validate-pr)

1. Fetch branches: `git fetch origin {base} {head}`
2. Get diff: `git diff origin/{base}...origin/{head}`
3. Get changed files: `git diff --name-only origin/{base}...origin/{head}`
4. Read each changed file in full for context
5. For each AC: check whether the diff and changed files satisfy the criterion
6. Return results in the status block

#### Return Protocol (validate-pr)

```
agent: qa
status: success | failed
output: pr-validation (inline)
summary: {N}/{total} AC passed
ac_results:
- AC-1: PASS — `file:line` — evidence
- AC-2: FAIL — `file:line` — evidence
...
In **validate-pr mode**, do not write to session-docs — return results inline via the status block.
